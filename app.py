...
with t2:
    ...
    # Initialize session state tracking for inputs so they can be explicitly cleared later
    if "selected_pool" not in st.session_state:
        st.session_state["selected_pool"] = []
    if "input_roster_name" not in st.session_state:
        st.session_state["input_roster_name"] = ""
        
    entry_name = st.text_input(
        "Enter Roster Submission Name:", 
        value=st.session_state["input_roster_name"],
        placeholder="e.g., Sarah - Lineup 1"
    ).strip()
    
    # Capture updates to text input back into session state
    st.session_state["input_roster_name"] = entry_name
    ...
    # MOVED SUBMISSION VALIDATION LOGIC UP
    can_submit = True
    if count_picked != 8:
    ...
    if st.button("Submit Official Roster Lineup", type="primary", disabled=not can_submit):
        ...
        # CLEAR EVERYTHING FOR THE NEXT USER
        st.session_state["selected_pool"] = []
        st.session_state["input_roster_name"] = ""
        st.success("Lineup successfully logged! Clear and ready for the next submission.")
        st.rerun()

    st.write("---")
    
    # Driver List Grid Configurations
