...
# --- VIEW 1: STANDINGS OVERVIEW ---
with t1:
    st.header("🏆 Master Pool Standings")
    master_standings = calculate_master_standings()
    
    if master_standings.empty:
        st.info("Standings will update dynamically once the first official roster is locked in.")
    else:
        # Determine current active race status column to sort pool rankings correctly
        if not (df['Pos_Final'] == 0).all():
            display_df = master_standings.sort_values(by="Final Pts", ascending=True)
            rank_col = "Final Place"
            score_col = "Final Pts"
        elif not (df['Pos_150'] == 0).all():
            display_df = master_standings.sort_values(by="150L Pts", ascending=True)
            rank_col = "150L Place"
            score_col = "150L Pts"
        elif not (df['Pos_100'] == 0).all():
            display_df = master_standings.sort_values(by="100L Pts", ascending=True)
            rank_col = "100L Place"
            score_col = "100L Pts"
        else:
            display_df = master_standings.sort_values(by="Starting Pts", ascending=True)
            rank_col = "Start Place"
            score_col = "Starting Pts"

        # Format and map the dynamic results to the optimized white-background dataframe view
        display_preview = display_df[[rank_col, "Name", score_col, "Starting Pts", "100L Pts", "150L Pts", "Final Pts"]].copy()
        display_preview.columns = ["Rank", "Participant Name", "Current Score", "Grid Score", "Lap 100", "Lap 150", "Final"]
        
        st.dataframe(display_preview, use_container_width=True, hide_index=True)
        st.caption("ℹ️ Lowest total score wins (Sum of all 8 roster driver positions).")
